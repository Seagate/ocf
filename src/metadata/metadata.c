	...
	ocf_cache_line_size_t line_size)
{
	...
	case metadata_segment_collision:
		size = sizeof(struct ocf_metadata_map) + ocf_metadata_status_sizeof(line_size);
		break;
	...
	ocf_cache_line_size_t line_size)
{
	...
	do
	{
			 ...
			 i < metadata_segment_max; i++)
		{
			...
			raw->entries = ocf_metadata_get_entries(i, cache_lines);
...
static void ocf_metadata_deinit_fixed_size(struct ocf_cache *cache)
{
	...
	{
		cache->user_parts[i].part.runtime = NULL;
		cache->user_parts[i].part.eviction_runtime = NULL;
	}
	...
	bool metadata_volatile)
{
	...
	for (i = 0; i < metadata_segment_fixed_size_max; i++)
	{
		...
		raw->entry_size = ocf_metadata_get_element_size(i, 0);
									...
									bool cleaner_disabled)
{
	...
	ctrl->mapping_size = ocf_metadata_status_sizeof(line_size) + sizeof(struct ocf_metadata_map);
	...
	OCF_DEBUG_PARAM(cache, "Metadata end pages = %u", ctrl->start_page + ocf_metadata_get_pages_count(cache));
	...
	for (i = 0; i < metadata_segment_max; i++)
	{
		ocf_cache_log(cache, log_info, "%s offset : %llu kiB\n",
					  ocf_metadata_segment_names[i],
					  ctrl->raw_desc[i].ssd_pages_offset * PAGE_SIZE / KiB);
		if (i == metadata_segment_sb_config)
		{
			ocf_cache_log(cache, log_info, "%s size : %lu B\n",
						  ocf_metadata_segment_names[i],
						  offsetof(struct ocf_superblock_config, checksum) + sizeof(((struct ocf_superblock_config *)0)
																						->checksum));
		}
		...
		else
		{
			ocf_cache_log(cache, log_info, "%s size : %llu kiB\n",
						  ocf_metadata_segment_names[i],
						  ctrl->raw_desc[i].ssd_pages * PAGE_SIZE / KiB);
		}
	}
...
finalize:
	...
	result = ocf_metadata_concurrency_attached_init(&cache->metadata.lock,
													...
													(uint32_t)ctrl->raw_desc[metadata_segment_collision].ssd_pages);
	if (result)
	{
												   ...
												   uint64_t sector_addr, uint32_t sector_no, ctx_data_t *data)
{
	...
	for (i = 0; i < sector_no; i++)
	{
		...
		if (meta.valid && core_id != OCF_CORE_ID_INVALID)
		{
			...
			meta.dirty ? metadata_set_dirty_sec_one(cache, line, pos) : metadata_clear_dirty_sec_one(cache, line, pos);
		}
	}
